  ...
  try {
    ...
    if (customerId && typeof customerId === "string") {
      ...
      const customer = customers?.find((c) => {
        const transformedId = parseInt(
          c.id.replace(/[^0-9]/g, "").substring(0, 8) || "1",
        );
    ...
    const transformedData =
      data?.map((transaction) => ({
        id: parseInt(
          transaction.id.replace(/[^0-9]/g, "").substring(0, 8) || "1",
        ),
        receiptNumber: `TXN${String(parseInt(transaction.id.replace(/[^0-9]/g, "").substring(0, 8) || "1")).padStart(6, "0")}`,
        customerId: transaction.customer_id,
        customerName: (transaction.customers as any)?.name || "Pelanggan Umum",
        customerPhone: (transaction.customers as any)?.phone || "",
        staffName: "No Staff",
...
router.post("/", async (req, res): Promise<void> => {
  try {
    ...
    const { customerId, staffId, items, discount, tax, paymentMethod } =
      requestData;
    ...
    const transformedData = {
      ...
      customerName: (result.customers as any)?.name || "Pelanggan Umum",
      customerPhone: (result.customers as any)?.phone || "",
...
router.get("/:id", async (req, res): Promise<void> => {
  try {
    ...
    const transaction = transactions.find((t) => {
      const transformedId = parseInt(
        t.id.replace(/[^0-9]/g, "").substring(0, 8) || "1",
      );
    ...
    const transformedData = {
      ...
      customerName: (transaction.customers as any)?.name || "Pelanggan Umum",
      customerPhone: (transaction.customers as any)?.phone || "",
      staffName: (transaction.staff as any)?.name || "No Staff",
      items: transaction.items || [],
      subtotal: Number(transaction.subtotal || 0),
      discount: Number(transaction.discount || 0),
      tax: Number(transaction.tax || 0),
