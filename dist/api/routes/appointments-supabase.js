    ...
    try {
      ...
      `)
        ...
        const transformedData = data?.map((appointment) => ({
            id: parseInt(appointment.id.replace(/[^0-9]/g, "").substring(0, 8) || "1"),
            customerName: appointment.customers?.name ||
                appointment.customer_name ||
                "Unknown",
            customerPhone: appointment.customers?.phone || appointment.customer_phone || "",
            serviceName: appointment.services?.name || "Unknown Service",
            servicePrice: Number(appointment.services?.price || 0),
            staffName: appointment.staff?.name || "Anyone",
            staffId: appointment.staff_id
                ? parseInt(appointment.staff_id.replace(/[^0-9]/g, "").substring(0, 8) || "1")
                : null,
            serviceId: parseInt(appointment.service_id.replace(/[^0-9]/g, "").substring(0, 8) || "1"),
...
router.post("/", async (req, res) => {
    try {
        const { customerName, customerPhone, serviceId, staffId, scheduledAt, notes } = req.body;
      ...
      `)
        ...
        const transformedData = {
            ...
            customerName: data.customers?.name || customerName,
            customerPhone: data.customers?.phone || customerPhone,
            serviceName: data.services?.name || service.name,
            servicePrice: Number(data.services?.price || service.price),
            staffName: data.staff?.name || "Anyone",
            staffId: data.staff_id
                ? parseInt(data.staff_id.replace(/[^0-9]/g, "").substring(0, 8) || "1")
                : null,
            serviceId: parseInt(data.service_id.replace(/[^0-9]/g, "").substring(0, 8) || "1"),
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const appointment = appointments.find((a) => {
            const transformedId = parseInt(a.id.replace(/[^0-9]/g, "").substring(0, 8) || "1");
      ...
      `)
        ...
        const transformedData = {
            ...
            servicePrice: Number(data.services?.price || 0),
            staffName: data.staff?.name || "Anyone",
            staffId: data.staff_id
                ? parseInt(data.staff_id.replace(/[^0-9]/g, "").substring(0, 8) || "1")
                : null,
